...
class ListingService {
  async getAll(params?: ListingSearchParams): Promise<Listing[]> {
    const response = await api.get<ListingResponse>(API_ENDPOINTS.LISTING.BASE, { params });
  ...
  async getByAgentId(agentId: string, params?: ListingSearchParams): Promise<Listing[]> {
    const response = await api.get<ListingResponse>(API_ENDPOINTS.LISTING.BY_AGENT(agentId), { params });
    return response.data.data;
  }

  async getByLocation(latitude: number, longitude: number, maxDistanceKm: number = 5): Promise<Listing[]> {
    const response = await api.get<ListingResponse>(API_ENDPOINTS.LISTING.BY_LOCATION, {
  ...
  async getByAddress(keyword: string): Promise<Listing[]> {
    const response = await api.get<ListingResponse>(API_ENDPOINTS.LISTING.BY_ADDRESS, {
      params: { keyword },
    });
    return response.data.data;
  }
